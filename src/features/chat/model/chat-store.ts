    ...
    connect: (chatId: string, userId: string) => {
        ...
        const fetchMessages = async () => {
            try {
                ...
                if (apiMessages.length > 0) {
                    console.log('[fetchMessages] Received:', apiMessages.length, 'messages');
        ...
        socket.on('newMessage', (msg) => {
            ...
            set((state) => {
                ...
                const potentialDuplicates = state.messages.filter(m => 
                    m.text === newMsg.text && 
                ...
                if (potentialDuplicates.length > 0) {
                    console.log('[newMessage] Potential duplicate content detected, skipping');
                    return state;
                }
        ...
        socket.on('messageHistory', (history) => {
            ...
            const formattedMessages = history.map((msg: any) => ({
                id: msg.id || `hist-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
                text: msg.text || '',
                date: new Date(msg.sentAt || Date.now()).toLocaleTimeString().slice(0, 5),
                isMy: msg.userId === userId,
                sentAt: msg.sentAt || Date.now(),
            ...
            set((state) => {
                ...
                console.log('[messageHistory] Adding', newMessages.length, 'new messages');
                return {
    ...
    _sendMessageImpl: (text: string, chatId: string, userId: string) => {
        ...
        socket.emit('sendMessage', {
            ...
            if (acknowledgement && acknowledgement.success) {
                console.log('[sendMessage] Message sent successfully:', acknowledgement);
            }
        });
    },
    ...
    sendMessage: (text: string, chatId: string, userId: string) => {
        ...
        if (!socket.connected) {
            ...
            socket.once('connect', () => {
                ...
                socket.emit('joinRoom', chatId); // Rejoin room after reconnection
