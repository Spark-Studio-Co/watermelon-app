...
export const BottomNavigationButton = ({ label, icon }: IBottomNavigatonButtonProps) => {
    ...
    return (
        <Button variant="custom" onPress={handlePressButton} className={`flex flex-row h-[48px] items-center gap-x-3 ${active === label ? `h-[48px] bg-[#2A2A2C] px-4 flex items-center justify-center rounded-tl-[15px] rounded-tr-[15px]` : 'w-[48px]'}`}>
            {icon}
            {active === label &&
