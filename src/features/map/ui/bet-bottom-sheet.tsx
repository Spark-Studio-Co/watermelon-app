...
export const BetBottomContent = () => {
  ...
  const { setStartBet, type, latitude, longitude, isPrivate } =
    useMarkerStore();
  ...
  const handleMakeBet = () => {
    if (isBetSet && type && (activeBet || value)) {
      ...
      data.append(
        "startingBid",
        String((activeBet || value).replace(/[^0-9.]/g, "").trim())
      );
  ...
  return (
    <View className="flex items-center flex-col h-[305px] w-[95%] mx-auto">
      ...
      <Text weight="bold" className="mt-5 text-white text-[20px]">
        Начальная ставка
      </Text>
      <View className="flex flex-row w-[75%] gap-x-2 justify-center items-center mt-5">
        ...
        <Button
          variant="custom"
          className={`px-4 py-3.5 rounded-[6px] ${
            isBetSet && type && (activeBet || value)
              ? "bg-[#14A278]"
              : "bg-[#14A278] opacity-50"
          }`}
          onPress={handleMakeBet}
          disabled={!type || (!activeBet && !value)}
        >
          <Text weight="bold" className="text-white text-[11.74px]">
            Make a bet
          </Text>
        </Button>
      </View>
        ...
        {bets.map((bet) => (
          ...
          >
            <Text
              ...
              style={{ color: active === bet ? "#FFFF" : "#000000" }}
            >
              {bet}
            </Text>
          </Button>
