  ...
  return (
    <MainLayout>
      <View className="flex flex-col mx-auto mt-5">
        <Text weight="medium" className="text-white text-[22px]">
          Settings
        </Text>
        <UserSettingsTab
      ...
      {friends.isVisible && (
        <View style={{ width: "100%" }}>
          <View className={`px-4 ${Platform.OS === "ios" ? "mt-10" : "mt-6"}`}>
            <SeettingsButton
              onPress={() => navigate("MyFriends" as never)}
              icon={<FriendsIcon />}
              label="Мои друзья"
              isFriendsSettings
            />
            <SeettingsButton
              onPress={() => navigate("SearchFriends" as never)}
              icon={<SearchIcon />}
              label="Искать друзей"
              className="mt-2"
            />
      ...
      {appSettings.isVisible && (
        <View style={{ width: "100%" }}>
          <View className={`px-4 ${Platform.OS === "ios" ? "mt-7" : "mt-1.5"}`}>
            ...
            <SeettingsButton
              onPress={() => navigate("PointPremium" as never)}
              icon={<CrownIcon />}
              label="Point Premium"
          ...
          <Button
            variant="custom"
            className="flex mx-auto mt-0.5 items-center justify-center gap-x-5 h-[68px] w-[95%] bg-[#262A34] rounded-[15px] flex-row"
          >
            <Text>
              <SendFeedBackIcon />
            </Text>
          ...
          <Button
            ...
            variant={"custom"}
            className="bg-red-500 flex mx-auto mt-4 items-center justify-center h-[68px] w-[95%] rounded-[15px]"
          >
            <Text weight="regular" className="text-white text-[20px]">
              Log out
            </Text>
