...
export const PrivateChatScreen = () => {
    ...
    // Debug messages
    useEffect(() => {
        console.log('[PrivateChatScreen] messages state updated:', messages.length, 
            messages.length > 0 ? `Latest: ${messages[messages.length-1]?.text}` : '');
    }, [messages]);
    ...
    return (
        ...
        >
            {isLoading ? (
                ...
                <View className="flex-1 justify-center items-center">
                    <Text className="text-gray-600">No messages yet. Start the conversation!</Text>
                </View>
            ) : (
