...
export const AuctionScreen = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useAuctionsData(undefined, filterMethod, statusMap[active as string]);
    ...
    if (Array.isArray(auctions)) {
      ...
      auctions.forEach((auction, i) => {
        if (!updatedMap[auction.id]) {
          updatedMap[auction.id] =
            `Point #${Object.keys(updatedMap).length + 1}`;
        }
      });
  ...
  return (
    <MainLayout isScrollable={false}>
      <View className="mt-7 flex flex-col">
        ...
        <View className="flex flex-row items-center justify-between w-[80%] mx-auto mt-7">
          {tabs.map((tab, index) => (
            <View key={index} className="flex flex-col items-center">
              <Text
                weight="regular"
                className="text-white text-[15px] mb-[3px]"
        ...
        {isLoading ? (
          <View className="flex items-center justify-center w-full h-[90%]">
                ...
                <Button
                  ...
                  onPress={() => {
                    ...
                    navigation.navigate("AuctionInner" as never, {
                      ...
                      name:
                        `Point #${auction?.marker?.map_id}` ||
                        nameMap[auction.id],
                      start: auction.currentBid,
            ...
            {filterOptions.map((option, index) => (
              ...
              >
                <View
                  style={[
                    styles.radioButton,
                    selectedFilter === index && styles.radioButtonSelected,
                  ]}
                />
                <Text weight="regular" className="text-black text-[16px] ml-3">
                  {option.label}
                </Text>
              </TouchableOpacity>
            ))}
